...
try:
    ...
    # Hash the new password using bcrypt with passlib
    print("\nHashing password...")
    hashed_password = hash.bcrypt.using(rounds=12).hash(new_password)
    
    # Update password in database
    cursor.execute("UPDATE user SET password = %s WHERE email = %s", (hashed_password, admin_email))
    conn.commit()
